...
class PolymarketHourlyTradingBot:
    ...
    async def load_historical_data(self, days: int = 30):
        ...
        self.logger.info(f"Loading {days} days of historical {TICKERS[self.config.market_slug]} data...")
        ...
        self.ohlcv_data["timestamp"] = pd.to_datetime(self.ohlcv_data["timestamp"], unit="ms", utc=True)
        self.ohlcv_data = self.ohlcv_data.set_index("timestamp").sort_index()
        self.ohlcv_data = self.ohlcv_data[~self.ohlcv_data.index.duplicated(keep="last")]
    ...
    async def refresh_markets(self):
        ...
        try:
            ...
            markets: list[Market] = [self.polymarket.get_market_by_slug(get_current_market_slug(self.config.market_slug))]
            for _ in range(1, self.config.max_markets_to_check):
                current_market_hour += timedelta(hours=1)
                next_market_slug = create_slug_from_datetime(current_market_hour, self.config.market_slug)
            ...
            order_books = self.polymarket.get_order_books([market.tokens[0].token_id for market in markets])
            ...
            self.logger.info(f"Found {len(active_markets)} active {self.config.market_slug} hourly markets")
    ...
    async def refresh_data(self):
        ...
        if (self.state.last_data_refresh and 
            (current_time - self.state.last_data_refresh).total_seconds() < self.config.data_refresh_interval):
            return
        ...
        try:
            ...
            if self.ohlcv_data is not None and not self.ohlcv_data.empty:
                ...
                since = self.exchange.parse8601((last_timestamp - timedelta(minutes=5)).isoformat())
                # Calculate how many minutes we need plus safety margin
                minutes_needed = int((current_time - last_timestamp).total_seconds() / 60) + 5
                limit = min(120, minutes_needed) # Cap at 120 minutes
            else:
                # If no data, get last 2 hours
                since = self.exchange.parse8601((current_time - timedelta(hours=2)).isoformat())
                limit = 120

            # Fetch data with overlap to ensure no missing minutes
            ohlcv = self.exchange.fetch_ohlcv(TICKERS[self.config.market_slug], "1m", since, limit)
            ...
            if ohlcv:
                ...
                new_data["timestamp"] = pd.to_datetime(new_data["timestamp"], unit="ms", utc=True)
                ...
                self.logger.debug(f"Refreshed {self.config.market_slug} data: {len(new_data)} new candles")
    ...
    def should_make_prediction(self) -> bool:
        ...
        in_prediction_window = (self.config.prediction_window_start <= 
                              minutes_elapsed <= 
    ...
    async def make_prediction(self) -> Optional[PredictionResult]:
        ...
        try:
            ...
            # Make prediction
            prediction = self.predictor.predict(self.ohlcv_data, pd.Timestamp(current_hour_start))
    ...
    def find_matching_markets(self, prediction: PredictionResult) -> List[tuple[Market, str]]:
        """Find markets that match our prediction and timing"""
        matching_markets = []
        current_time = datetime.now(timezone.utc)
        closes_in = (current_time.replace(hour=current_time.hour + 1, minute=0, second=0, microsecond=0) - current_time).total_seconds() / 60
            ...
            if market_slug == current_hour_slug:  # Market closes in 30-90 minutes
                ...
                if prediction.predicted_direction == "green":
                    ...
                    for token in market.tokens:
                        ...
                        if any(word in outcome_lower for word in ["up", "green", "yes", "higher", "above", "rise"]):
                    ...
                    for token in market.tokens:
                        outcome_lower = token.outcome.lower()
                        if any(word in outcome_lower for word in ["down", "red", "no", "lower", "below", "fall"]):
                            target_token_id = token.token_id
                            break
    ...
    async def analyze_order_book(self, token_id: str) -> dict:
        ...
        try:
            ...
            best_ask = min([float(ask["price"]) for ask in order_book.asks]) if order_book.asks else 1
            ...
            return {
                ...
                "midpoint": (best_bid + best_ask) / 2 if best_bid > 0 and best_ask < 1 else 0.5,
    ...
    def calculate_position_size(self, confidence: float, market_odds: float) -> float:
        ...
        position_size = min(position_fraction * self.config.max_position_size,
                          self.config.max_position_size * 0.5)  # Max 50% of max size per trade
        
        return max(position_size, 5)  # Enforce minimum $5 position size
    
    async def place_limit_order(self, market: Market, token_id: str, 
                              prediction: PredictionResult, order_book_info: dict) -> Optional[str]:
        """Place a limit order based on prediction and order book analysis"""
        try:
            # Check confidence threshold
            if prediction.confidence < self.config.confidence_threshold:
                self.logger.info(f"Confidence {prediction.confidence:.1%} below threshold {self.config.confidence_threshold:.1%}")
                return None
            ...
            if target_price <= best_bid:
                self.logger.info(f"Target price {target_price:.3f} not better than best bid {best_bid:.3f}")
                return None
            
            # Calculate position size
            position_size = self.calculate_position_size(prediction.confidence, market_prob)
            ...
            target_price = round(round(target_price / market.minimum_tick_size) * market.minimum_tick_size, 3)
            ...
            if order_id:
                ...
                position_tracker = PositionTracker(
                    ...
                    token_id=token_id,
                    token_outcome=next((token.outcome for token in market.tokens if token.token_id == token_id), "Unknown"),
        ...
        try:
            ...
            for order in open_orders:
                if order.order_id in self.state.orders_placed:
                    ...
                    order_time = datetime.fromtimestamp(order.created_at / 1000, tz=timezone.utc)
                    if (current_time - order_time).total_seconds() > (self.config.order_timeout_minutes * 60):
                        orders_to_cancel.append(order.order_id)
            
            if orders_to_cancel:
                self.logger.info(f"Cancelling {len(orders_to_cancel)} expired orders")
                removed_orders, failed_to_cancel = self.polymarket.cancel_orders(orders_to_cancel)
        ...
        try:
            ...
            for position in positions_to_resolve:
                ...
                if resolved_position:
                    ...
                    # Log the result
                    self.logger.info(f"Position resolved: {resolved_position.market_question}")
                    self.logger.info(f"  Outcome: {resolved_position.final_outcome}")
                    self.logger.info(f"  P&L: ${resolved_position.pnl:.2f}")
                    self.logger.info(f"  Win Rate: {self.state.winning_trades/self.state.total_trades:.1%}")
    ...
    async def _resolve_position(self, position: PositionTracker) -> Optional[PositionTracker]:
        ...
        try:
            ...
            if prediction_correct:
                ...
                pnl = position.position_size * (1 - position.entry_price) / position.entry_price
                final_outcome = "win"
            else:
    ...
    def save_position_data(self):
        ...
        try:
            ...
            data = {
                ...
                "resolved_positions": [pos.to_dict() for pos in self.state.resolved_positions],
                "open_positions": [pos.to_dict() for pos in self.state.open_positions.values()],
    ...
    async def run_trading_cycle(self):
        ...
        # Check confidence threshold
        if prediction.confidence < self.config.confidence_threshold:
            self.logger.info(f"Confidence {prediction.confidence:.1%} below threshold, skipping trading")
            return
        
        # Find matching markets
        matching_markets = self.find_matching_markets(prediction)
        if not matching_markets:
            self.logger.info(f"No matching {self.config.market_slug} markets found for prediction")
            return
        
        self.logger.info(f"Found {len(matching_markets)} matching {self.config.market_slug} markets")
        ...
        for market, token_id in matching_markets:
            try:
                ...
                total_liquidity = order_book_info.get("bid_liquidity", 0) + order_book_info.get("ask_liquidity", 0)
                if total_liquidity < self.config.min_liquidity:
                    self.logger.info(f"Market liquidity ${total_liquidity:.2f} below minimum")
                    continue
                
                # Place order
                order_id = await self.place_limit_order(market, token_id, prediction, order_book_info)
                if order_id:
            ...
            try:
                ...
                if current_time.minute == 0:
                    ...
                    if current_time.hour % 1 == 0:
                        summary = self.get_performance_summary()
                        self.logger.info(f"Performance Update: {summary['total_trades']} trades, "
