...
try:
    ...
    print(
        "Moving average variables are not defined. Calculation likely failed in previous steps."
    )
except Exception as e:
...
def plot_resampled_with_ma(df, ma, title):
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df, mode="lines", name="Close Price"))
...
print(
    f"Probability of a candle flipping from DOWN (< -{threshold}%) at 45min to UP at close: {prob_flip_from_down:.2%}"
)
