...
class AlternativeDataFetcher:
    ...
    async def fetch_polymarket_markets_gamma(
        self, keyword: str = "ethereum"
    ) -> List[Dict]:
    ...
    async def get_market_health_check(self) -> Dict[str, bool]:
        ...
        health = {"coingecko": False, "coinbase": False, "polymarket_gamma": False}
    ...
    async def get_eth_ohlcv_data(self, hours: int = 2) -> Optional[pd.DataFrame]:
        ...
        if self.ccxt_exchange:
            try:
                ...
                target_candles = (
                    hours * 60
                )  # Approximate number of 1-minute candles needed

                while True:
                    try:
                        ohlcv = self.ccxt_exchange.fetch_ohlcv(
                            "ETH/USDT", "1m", since, limit
                        )
