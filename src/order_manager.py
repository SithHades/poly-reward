...
from src.strategy import PolymarketLiquidityStrategy, OrderbookSnapshot, LiquidityProvisionConfig
...
class OrderMetrics:
    ...
    def update_fill_rate(self):
        ...
        if self.total_orders_placed > 0:
            self.fill_rate = Decimal(self.total_orders_filled) / Decimal(self.total_orders_placed)
...
class ManagedOrder:
    ...
    lifecycle_events: List[Tuple[datetime, OrderLifecycleEvent, str]] = field(default_factory=list)
...
class OrderManager:
    ...
    async def place_order(
        ...
        if len(self.active_orders) >= self.max_active_orders:
            self.logger.warning(f"Maximum active orders reached: {self.max_active_orders}")
            return None
        ...
        try:
            ...
            self.logger.info(f"Placing order: {side.value} {size} at {price} for token {token_id}")
            ...
            # Create order object
            order_id = response.get("id", f"order_{len(self.active_orders)}")
            ...
            managed_order.add_lifecycle_event(OrderLifecycleEvent.PLACED, f"Placed {side.value} {size} at {price}")
    ...
    async def update_order_status(self, order_id: str) -> bool:
        ...
        try:
            ...
            if current_status != old_status or filled_size != managed_order.order.filled_size:
                ...
                if current_status != old_status:
                    event = OrderLifecycleEvent.FILLED if current_status == OrderStatus.FILLED else \
                           OrderLifecycleEvent.PARTIALLY_FILLED if current_status == OrderStatus.PARTIALLY_FILLED else \
                           OrderLifecycleEvent.CANCELLED
                    
                    managed_order.add_lifecycle_event(event, f"Status: {old_status.value} -> {current_status.value}")
                    self.logger.info(f"Order {order_id} status changed: {old_status.value} -> {current_status.value}")
    ...
    async def manage_order_lifecycle(self, market_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            market_orders = [mo for mo in self.active_orders.values() 
                           if mo.order.market_id == asset_id]
            
            orders_to_cancel = self.strategy.should_cancel_orders(
                asset_id, 
                [mo.order for mo in market_orders]
            )
            ...
            if yes_orderbook and no_orderbook:
                ...
                for order_spec in new_orders:
                    order_id = await self.place_order(
                        ...
                        metadata={"market_type": order_spec.get("market_type", ""), "reason": order_spec.get("reason", "")}
                    )
    ...
    def get_order_status_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "positions": {market_id: {"size": float(pos.size), "entry_price": float(pos.entry_price)} 
        ...
        return [
            {
                ...
                "age_minutes": (datetime.now(timezone.utc) - managed_order.placement_time).total_seconds() / 60,
    ...
    def _update_position(self, order: Order):
        ...
        if position.size != 0:
            position.entry_price = order.price  # Simplified - should be weighted average
    ...
    async def _generate_hedge_orders(self, managed_order: ManagedOrder):
        ...
        try:
            ...
            self.logger.info(f"Hedge order generation needed for {managed_order.order.id}")
    ...
    def _validate_order(self, price: Decimal, size: Decimal, side: OrderSide, token_id: str) -> bool:
