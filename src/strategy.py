...
import logging
from src.models import Order, Position, OrderSide
from src.api_models import Market as ApiMarket
...
class OrderbookSnapshot:
    ...
    def total_bid_volume_in_range(self, price_range: Tuple[Decimal, Decimal]) -> Decimal:
        """Calculate total bid volume within price range"""
        min_price, max_price = price_range
        return sum(level.size for level in self.bids 
                  if min_price <= level.price <= max_price)
    
    def total_ask_volume_in_range(self, price_range: Tuple[Decimal, Decimal]) -> Decimal:
...
class VolatilityMetrics:
    ...
    def is_volatile(self, midpoint_threshold: Decimal = Decimal("0.02"), 
                   spread_threshold: Decimal = Decimal("0.05")) -> bool:
        ...
        if self.midpoint_changes:
            recent_midpoint_data = self.midpoint_changes[-10:] if len(self.midpoint_changes) >= 10 else self.midpoint_changes
            recent_midpoint_volatility = max(abs(change) for change in recent_midpoint_data)
        else:
            recent_midpoint_volatility = Decimal("0")
        
        # Check for recent spread expansion
        if self.spread_changes:
            recent_spread_data = self.spread_changes[-10:] if len(self.spread_changes) >= 10 else self.spread_changes
            recent_spread_volatility = max(abs(change) for change in recent_spread_data)
        else:
            recent_spread_volatility = Decimal("0")
        
        return (recent_midpoint_volatility > midpoint_threshold or 
                recent_spread_volatility > spread_threshold)
...
class LiquidityProvisionConfig:
    ...
    volatility_exit_threshold: Decimal = Decimal("0.02")  # 2% midpoint move triggers exit
...
class PolymarketLiquidityStrategy:
    ...
    def analyze_market_condition(self, yes_orderbook: OrderbookSnapshot, 
                                no_orderbook: OrderbookSnapshot) -> MarketCondition:
        """
        ...
        try:
            ...
            if self._is_market_volatile(yes_orderbook.asset_id, yes_orderbook, no_orderbook):
                return MarketCondition.VOLATILE
            
            # Analyze competition in the spread
            competition_density = self._calculate_competition_density(yes_orderbook, no_orderbook)
            if competition_density > self.config.max_competition_density:
                self.logger.info(f"Market {yes_orderbook.asset_id} too competitive: {competition_density:.2%} density")
                return MarketCondition.COMPETITIVE
            
            # Estimate potential reward share
            potential_reward_share = self._estimate_reward_share(yes_orderbook, no_orderbook)
            if potential_reward_share < self.config.min_reward_share_threshold:
                self.logger.info(f"Market {yes_orderbook.asset_id} low reward share: {potential_reward_share:.2%}")
                return MarketCondition.COMPETITIVE
            
            # Market looks attractive for LP
            self.logger.info(f"Market {yes_orderbook.asset_id} attractive: {competition_density:.2%} density, {potential_reward_share:.2%} reward share")
    ...
    def calculate_optimal_orders(self, yes_orderbook: OrderbookSnapshot, 
                               no_orderbook: OrderbookSnapshot,
                               current_positions: Dict[str, Position],
                               available_capital: Decimal) -> List[Dict[str, Any]]:
        """
        ...
        if market_condition != MarketCondition.ATTRACTIVE:
            self.logger.info(f"Skipping orders for {yes_orderbook.asset_id}: {market_condition.value}")
            return []
        ...
        if max_order_size < self.config.min_order_size:
            self.logger.warning(f"Insufficient capital for minimum order size: {max_order_size} < {self.config.min_order_size}")
            return []
        
        # Generate YES market orders
        yes_orders = self._generate_market_orders(
            yes_orderbook, True, max_order_size, current_positions.get(yes_orderbook.asset_id)
        )
        ...
        if self.config.enable_yes_no_hedging:
            no_orders = self._generate_market_orders(
                no_orderbook, False, max_order_size, current_positions.get(no_orderbook.asset_id)
            )
            orders.extend(no_orders)
        
        return orders
    
    def should_cancel_orders(self, asset_id: str, current_orders: List[Order]) -> List[str]:
        """
        ...
        for order in current_orders:
            ...
            if order_age > timedelta(minutes=self.config.max_order_age_minutes):
                ...
                self.logger.info(f"Cancelling old order {order.id}: {order_age.total_seconds()/60:.1f} minutes old")
                continue
            
            # Cancel if market became volatile
            if asset_id in self.volatility_tracker and self.volatility_tracker[asset_id].is_volatile():
                orders_to_cancel.append(order.id)
    ...
    def _is_market_volatile(self, asset_id: str, yes_orderbook: OrderbookSnapshot, 
                           no_orderbook: OrderbookSnapshot) -> bool:
    ...
    def _calculate_competition_density(self, yes_orderbook: OrderbookSnapshot, 
                                     no_orderbook: OrderbookSnapshot) -> Decimal:
        """
        ...
        yes_volume_in_range = (
            yes_orderbook.total_bid_volume_in_range(spread_range) +
            yes_orderbook.total_ask_volume_in_range(spread_range)
        )
        
        no_volume_in_range = (
            no_orderbook.total_bid_volume_in_range(spread_range) +
        ...
        estimated_capacity = self.config.min_order_size * 10  # Assume 10x min order size as capacity
        
        return min(total_volume_in_range / estimated_capacity, Decimal("1.0"))
    
    def _estimate_reward_share(self, yes_orderbook: OrderbookSnapshot, 
                             no_orderbook: OrderbookSnapshot) -> Decimal:
        """
        ...
        # Estimate our share based on minimum order size vs total volume
        our_planned_volume = self.config.min_order_size * 2  # YES + NO orders
        
        # Simple heuristic: our share inversely related to competition density
        estimated_share = (Decimal("1.0") - competition_density) * Decimal("0.5")  # Max 50% in best case
        
        return max(estimated_share, Decimal("0.01"))  # Minimum 1%
    
    def _generate_market_orders(self, orderbook: OrderbookSnapshot, is_yes_market: bool,
                               max_size: Decimal, current_position: Optional[Position]) -> List[Dict[str, Any]]:
    ...
    def calculate_hedge_orders(self, filled_order: Order, yes_orderbook: OrderbookSnapshot,
                              no_orderbook: OrderbookSnapshot) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            hedge_side = OrderSide.SELL if filled_order.side == OrderSide.BUY else OrderSide.BUY
            ...
            self.logger.info(f"Generated hedge order: {hedge_side.value} {hedge_size} at {hedge_price} in {hedge_market_type} market")
