...
from src.order_manager import OrderManager, ManagedOrder, OrderMetrics, OrderLifecycleEvent
from src.models import Order, Position, OrderSide, OrderStatus
from src.polymarket_client import PolymarketClient
from src.strategy import PolymarketLiquidityStrategy, LiquidityProvisionConfig, OrderbookSnapshot, OrderbookLevel
...
class TestOrderManager:
    def setup_method(self):
        ...
        self.mock_client.place_order.return_value = {"id": "test_order_123", "status": "placed"}
        self.mock_client.cancel_order.return_value = {"status": "cancelled"}
        self.mock_client.get_order_status.return_value = {"status": "open", "size_matched": "0"}
    ...
    async def test_place_order_max_orders_limit(self):
        ...
        for i in range(10):
            self.mock_client.place_order.return_value = {"id": f"order_{i}", "status": "placed"}
            order_id = await self.order_manager.place_order(
                price=Decimal("0.55"),
                size=Decimal("100"),
                side=OrderSide.BUY,
        ...
        self.mock_client.place_order.return_value = {"id": "order_overflow", "status": "placed"}
        order_id = await self.order_manager.place_order(
            price=Decimal("0.55"),
            size=Decimal("100"),
            side=OrderSide.BUY,
    ...
    async def test_place_order_paper_trading(self):
        ...
        self.mock_client.place_order.return_value = {"status": "paper", "price": 0.55, "size": 100, "side": "buy", "token_id": "token_123"}
    ...
    async def test_cancel_all_orders(self):
        ...
        # Cancel all orders
        cancelled_count = await self.order_manager.cancel_all_orders("mass_cancellation")
    ...
    def test_validate_order(self):
        ...
        # Valid order
        assert self.order_manager._validate_order(
...
class TestManagedOrder:
    def test_managed_order_creation(self):
        ...
        # Add lifecycle event
        managed_order.add_lifecycle_event(OrderLifecycleEvent.PLACED, "Order placed successfully")
