...
class TestOrderbookSnapshot:
    ...
    def test_volume_in_range_calculation(self):
        ...
        bid_volume = orderbook.total_bid_volume_in_range((Decimal("0.49"), Decimal("0.51")))
        assert bid_volume == Decimal("300")  # 100 + 200
        
        # Test ask volume in range 0.55-0.57
        ask_volume = orderbook.total_ask_volume_in_range((Decimal("0.55"), Decimal("0.57")))
...
class TestVolatilityMetrics:
    def test_volatility_tracking(self):
        ...
        assert metrics.is_volatile(Decimal("0.02"), Decimal("0.05"))  # Should be volatile
        assert not metrics.is_volatile(Decimal("0.05"), Decimal("0.1"))  # Should not be volatile
    ...
    def test_window_maintenance(self):
        ...
        assert metrics.midpoint_changes[0] == Decimal("0.010")  # Should start from 10th element
...
class TestPolymarketLiquidityStrategy:
    ...
    def test_competition_density_calculation(self):
        ...
        density = self.strategy._calculate_competition_density(self.yes_orderbook, self.no_orderbook)
        ...
        high_density = self.strategy._calculate_competition_density(high_competition_yes, self.no_orderbook)
        assert high_density > density  # Should be higher competition
    
    def test_market_condition_analysis(self):
        # Test attractive market
        condition = self.strategy.analyze_market_condition(self.yes_orderbook, self.no_orderbook)
        ...
        self.strategy.volatility_tracker["test_yes"].add_midpoint_change(Decimal("0.05"))  # Large change
    ...
    def test_order_cancellation_logic(self):
        ...
        orders_to_cancel = self.strategy.should_cancel_orders("test_asset", current_orders)
    ...
    def test_volatility_update(self):
        ...
        assert isinstance(self.strategy.volatility_tracker["test_asset"], VolatilityMetrics)
    
    def test_reward_share_estimation(self):
        # Test with low competition
        reward_share = self.strategy._estimate_reward_share(self.yes_orderbook, self.no_orderbook)
...
class TestIntegrationScenarios:
    ...
    def test_risk_management_triggers(self):
        ...
        self.strategy.volatility_tracker["volatile_test"].add_midpoint_change(Decimal("0.1"))
        
        # Should detect as volatile
        condition = self.strategy.analyze_market_condition(volatile_orderbook, volatile_orderbook)
        ...
        with patch.object(self.strategy.volatility_tracker["volatile_test"], 'is_volatile', return_value=True):
            orders_to_cancel = self.strategy.should_cancel_orders("volatile_test", test_orders)
