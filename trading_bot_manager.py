...
class TradingBotManager:
    ...
    def create_trading_config(self) -> TradingConfig:
        ...
        return TradingConfig(
            ...
            prediction_window_start=data_config.get("prediction_window", {}).get(
                "start_minute", 43
            ),
            prediction_window_end=data_config.get("prediction_window", {}).get(
                "end_minute", 47
            ),
        )
    ...
    async def test_prediction(self):
        ...
        try:
            # Create predictor
            model_config = self.config.get("model", {})
            _ = EthCandlePredictor(model_type=model_config.get("type", "logistic"))
            ...
            if prediction.confidence >= threshold:
                print(
                    f"✅ TRADE - Confidence {prediction.confidence:.1%} >= {threshold:.1%}"
                )
            else:
                print(
                    f"❌ NO TRADE - Confidence {prediction.confidence:.1%} < {threshold:.1%}"
                )
    ...
    async def check_markets(self, market_slug: MARKETS):
        ...
        try:
            ...
            markets = [
                client.get_market_by_slug(current_market_slug),
                client.get_market_by_slug(next_market_slug),
            ]
            ...
            for market in markets:
                print(
                    f"    Ends: {market.end_date_iso.strftime('%Y-%m-%d %H:%M:%S') if market.end_date_iso else 'N/A'}"
                )
    ...
    async def check_positions(self):
        ...
        try:
            ...
            if (total_reserved_exposure + total_exposure) > max_exposure:
                print(
                    f"⚠️  WARNING: Total reserved exposure ${total_reserved_exposure:.2f} + total exposure ${total_exposure:.2f} exceeds limit ${max_exposure:.2f}"
                )
            else:
                print("✅ Risk OK: Total exposure within limits")
...
async def main():
    ...
    subparsers.add_parser(
        "test-prediction", help="Test prediction model with current data"
    )

    # Add check-markets command with slug argument
    check_markets_parser = subparsers.add_parser(
        "check-markets", help="Check available Polymarket markets"
    )
